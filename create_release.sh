...
create_lib_release(){
    ...
    local RELEASE_NAME=libtempli_archlinux@1.0.1
...
create_cli_release(){
    local RELEASE_NAME=templi_cli_archlinux@1.0.1
    release_path+=("${RELEASE_NAME}")
...
git add --all
git commit -m "release: libtempli@1.0.1 - templi_cli@1.0.1"
git tag -a v3.2.1 -m "templi v3.2.1"
